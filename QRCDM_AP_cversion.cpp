...
void main_func(double * x, double * record, double * final_gap,
    ...
    double * lambda = new double [maxe]();
    double * y_sum = new double [N]();
    double * lambda_sum = new double [N]();
    double * phi = new double [R](); 
    double * total_degree_vec = new double [N]();
    double * decomposed_W = new double [N]();
    double * temp_W = new double [maxe]();
    double * b = new double [N]();
    int i, j, t;
    double gap;
    for(i = 0; i < R; i++){
        y[i] = new double [incidence_list_size[i]]();
        //lambda = new double [incidence_list_size[i]]();
    }
    degree_stat(total_degree_vec, incidence_list, incidence_list_size, N, R);
    for(i = 0; i < N; i++){
        x[i] = 0;
        b[i] = 2*W[i]*bias_vec[i];
        decomposed_W[i] = W[i]/total_degree_vec[i];
    }
    for(t = 0; t <= T; t++){
        if(t%record_dis == 0){
            for(i = 0; i < N; i++){
...
//                 bias_vec, x, W, N, R);  
        ...
        for(i = 0; i < N; i++){
            lambda_sum[i] =  (y_sum[i] - b[i])/total_degree_vec[i];
            y_sum[i] = 0;
        }
        for(i = 0; i < R; i++){
            for(j = 0; j < incidence_list_size[i]; j++){
                lambda[j] = y[i][j] - lambda_sum[incidence_list[i][j]];
                temp_W[j] = decomposed_W[incidence_list[i][j]];
                //weight_lambda[j] = lambda[i][j]/temp_W[j];
            }
            if(submodular_type[i][0] == 'h'){
                projection_undirected_hyperedge(y[i], & phi[i], lambda, temp_W, parameter_list[i], incidence_list_size[i]);
            }
            for(j = 0; j < incidence_list_size[i]; j++){
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    // [y, record] = QRCDM_AP_cversion(incidence_list, parameter_list, submodular_type, bias_vec, W, N, R, T, record_dis);
